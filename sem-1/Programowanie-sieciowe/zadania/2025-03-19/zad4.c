...
int main(int argc, char** argv) {
  ...
  if (argc != 3) {
    ...
    printf("  0 - wyłącz tryb promiscuous\n");
    printf("  1 - włącz tryb promiscuous\n");
    return EXIT_FAILURE;
  }
  ...
  // Przetwarzanie argumentu włączenia/wyłączenia trybu promiscuous
  long onoff = strtol(argv[2], NULL, 10);
  if (onoff == 1) {
    ifr.ifr_flags |= IFF_PROMISC;
    printf("Włączanie trybu promiscuous dla interfejsu %s\n", argv[1]);
  } else if (onoff == 0) {
    ifr.ifr_flags &= ~IFF_PROMISC;
    printf("Wyłączanie trybu promiscuous dla interfejsu %s\n", argv[1]);
  } else {
